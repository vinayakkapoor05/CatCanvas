...
window.addEventListener('DOMContentLoaded', () => {
  ...
  const clearChatBtn = getEl('clearChatBtn'); // Add this element to your HTML

  // —— CONVERSATION MEMORY MANAGEMENT ——
  // Store conversation IDs in localStorage
  const CHAT_CONVERSATION_ID_KEY = 'chatConversationId';
  const PLAN_CONVERSATION_ID_KEY = 'planConversationId';
  
  // Initialize or retrieve conversation IDs
  let chatConversationId = localStorage.getItem(CHAT_CONVERSATION_ID_KEY);
  let planConversationId = localStorage.getItem(PLAN_CONVERSATION_ID_KEY);
  ...
  async function clearConversationMemory(conversationId) {
    if (!conversationId) return;
    
    try {
      const res = await fetch(`${API_BASE}/api/rag/clear_memory?conversation_id=${conversationId}`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${API_TOKEN}`
        }
  ...
  if (clearChatBtn) {
    clearChatBtn.addEventListener('click', async () => {
      try {
        ...
        const conversationId = isPlan ? planConversationId : chatConversationId;
        const statusSetter = isPlan ? setPlanStatus : setChatStatus;
        const responseSetter = isPlan ? setPlanResponse : setChatResponse;
        
        statusSetter('Clearing conversation...');
        
        if (conversationId) {
          await clearConversationMemory(conversationId);
          
          // Reset the conversation ID in memory and localStorage
          if (isPlan) {
            planConversationId = null;
            localStorage.removeItem(PLAN_CONVERSATION_ID_KEY);
          } else {
          ...
          responseSetter('');
          statusSetter('Conversation cleared');
        } else {
          statusSetter('No active conversation to clear');
          setTimeout(() => statusSetter(''), 2000);
        }
      } catch (err) {
        const isPlan = planTabContent.classList.contains('active');
        const statusSetter = isPlan ? setPlanStatus : setChatStatus;
        statusSetter(`Error clearing conversation: ${err.message}`);
      }
    });
  }
  ...
  if (queryBtn) {
    queryBtn.addEventListener('click', async () => {
      ...
      // Get the current conversation ID (or leave as null for first message)
      const conversationId = isPlan ? planConversationId : chatConversationId;
      ...
      // Check for conversation ID in the response headers and store it
      const responseConversationId = res.headers.get('X-Conversation-ID');
      if (responseConversationId) {
        if (isPlan) {
          planConversationId = responseConversationId;
          localStorage.setItem(PLAN_CONVERSATION_ID_KEY, responseConversationId);
        } else {
          chatConversationId = responseConversationId;
          localStorage.setItem(CHAT_CONVERSATION_ID_KEY, responseConversationId);
        }
      }
